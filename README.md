...
**watchFBL.com.** GitHub Pages serves one custom domain per repository, so point watchFBL.com at the watch page with a registrar-level forward (301 redirect) from `watchfbl.com` and `www.watchfbl.com` to `https://fusionballleague.com/watch/`. If you would rather it resolve directly to YouTube, forward it to the channel, https://www.youtube.com/@FusionballLeague, instead.
